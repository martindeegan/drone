...
\addplot[samples=200, domain=-10:10] {sin(deg(x))};
\addplot[samples=200, blue] {(x)};
\addplot[samples=200, red] {0 + cos(0)(x)-sin(0)/2!*(x)^2-cos(0)/3!*(x^3)};
\addplot[samples=200, orange] {0 + cos(0)(x)-sin(0)/2!*(x)^2-cos(0)/3!*(x^3)+cos(0)/5!*(x^5)};
\legend{$f(x) = \sin(x)$, 1st order, 3rd order, 5th order}
\end{axis}
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[
    ...
    \addplot3 [surf, opacity=0.25, black] {Z(x,y)};
    \addplot3 [surf, opacity=0.25, red] {Z(1,1) + Z_x(1,1)*(x-1) + Z_y(1,1)*(y-1) + (1/2)*Z_xx(1,1)*(x-1)^2 + (1/2)*Z_yy(1,1)*(y-1)^2};
    \legend{$f(x)$, First Order}
    \end{axis}
\end{tikzpicture}
\end{center}
